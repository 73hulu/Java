...
一般来说，这个负载因子取值为0.75，这个数提供了时间和空间成本之间的良好折衷。如果过大，可以减少空间开销但是会增加查找成本，反应在大部分的HashMap类的操作中，包括get和put操作。在设置初始容量的时候，应该考虑映射中条目数量及其负载因子，以尽量减少重新操作的次数。如果初始容量大于最大入口数量除以负载因子，则不会发生重新刷新的操作。
