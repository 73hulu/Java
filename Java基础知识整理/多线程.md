...
### 实现Callable接口
`Callable`接口是定义在java.util.concurrent.locks中的接口，和`Runnable`非常类似，都表示一项任务，能够让线程运行，也是一个函泛型参数化接口、数式接口，里面只有一个`call`方法，该方法可能会抛出异常`Exception`。
...
|  受影响的线程 |  当前线程 | 在该对象上wait的随机一个线程  |在该对象上wait的所有线程   |  当前线程  | 当前线程   | 当前线程   |
