...
堆是一个特殊的完全二叉树，满足根节点全部大于或小于左右子树的节点值。`PriorityQueue`使用数组实现该节点，默认为最小堆，如果要创建最大对，可以在创建的时候传入自定义的`Comparator`。需要注意注意的是，`PriorityQueue`不禁止null元素，否则抛出空指针异常。
...
`PriorityQueue`的删除操作可以是将根节点删除（或者软删除，即将其放到数组的默认，由于size减1，所以之后无法访问到，相当于删除了），其做法是将最后一个元素赋值到根节点中，然后堆就变成了这样的结构：根的左右子树是堆，但是整体不是堆，这时候只需要将根节点与左右子树中较小的那个数交换位置，然后递归循环，直到跟根节点比左右孩子节点值都小或者达到叶子节点。
...
3. 数据存储结构：`HashMap`采取“数组 + 单项链表 + 红黑树” 结构，而`HashTable`采取"数组 + 单项链表"。
...
`fixAfterDeletion`对删除节点后的红黑树进行调整和着色，过程比插入更加复杂，仍旧是看红黑树博文和`TreeMap`源码解读，再次不再赘述。
...
`Arrays`提供`asList`方法，将数组转为`ArrayList`实例，其逆操作是List实例的`toArray`方法。
