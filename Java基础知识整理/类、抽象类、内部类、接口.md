...
相对于成员内部类，静态内部类多了一个static修饰词。**静态内部类是不需要依附外部类的**，可以将其看做是类的静态变量。并且他不能使用外部类的非static成员变量或方法，这点很好理解，因为在没有外部类的对象的情况下，可以创建静态内部类的对象，如果允许访问外部类非static成员就会产生矛盾，因为外部类的非static成员必须依附具体的对象。
